...
func NewCachingFeed(f *Feed) *CachingFeed {
	CachingFeedInstance := CachingFeed{
		&Feed{
	...
	return &CachingFeedInstance // Remove this line.
}
...
func (cf *CachingFeed) Fetch(key string) (Document, error) {
	...
	doc := Document{
		Key:   key,
		Title: "TitleCachingFeed for " + key,
	}
	...
	return doc, nil // Remove this line.
}
...
func main() {
	...
	fmt.Println("Using CachingFeed directly")
	process(&CachingFeed{})
}
